    ...
    if request.method == 'POST': 
        ...
        if(out=="out"): 
            if 'id' in request.session:
                ...
                del request.session['ngays']
                del request.session['ngaye']
    ...
    if checkLogin(request): 
        ...
        if request.method == 'POST':
            ...
            if action == "loc": # thống kê theo lọc
                listT =thongBao(request)
                return render(request, 'pages/thongke.html',{"device":chuyenGioSangTiet(data[0]), "sum":data[1],"name":nameUser,"role":rl,"thongbao":listT,"id":id,"mon":timKiem['mon'], "tu":timKiem['tu'], "den":timKiem['den'], "giaovien":timKiem['giaovien']})
            if action == "lvx": 
                datal =locCuaLoc(timKiem)
                file = xuatDanhSachTheoLoc(chuyenGioSangTiet(datal[0]))
                return file
            if e != None: 
